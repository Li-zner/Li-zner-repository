...
以 JWT 为例（按下列步骤自行实现脚本，核心是双 Key 读取与并行期校验）：
...
- 轮换落地：[[MCP sever/JWT令牌签发与校验/README|JWT 令牌签发与校验]]（双 Key 并行期的实现方）
- 验签机制参考：[[可复用代码/README|可复用代码]]（HMAC 签名+防重放）
- 上位视角：[[AI应用资产/安全审计|AI 应用安全审计]]（供应链与运维侧检查单引用本技能）
