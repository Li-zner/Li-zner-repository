...
KNOWLEDGE_CHUNKS = [
    ...
    },
    {
        ...
        "content": "多路召回架构：SQL关键词检索（pg_trgm）+ Embedding向量检索（pgvector，768维）并行执行。Cross-Encoder Rerank精排提升相关性。冷热分层记忆：Redis热缓存（L1）+ PostgreSQL长期归档（L2）+ 用户画像自动提取。pg_trgm语义缓存，高频问题毫秒级命中。RAGAS评测：60条专业用例综合得分0.925。",
    },
    {
        "heading": "服务治理体系",
        "content": "Redis分布式限流（IP+Token级别，角色分级）。熔断降级机制，自动切换备胎模型。SSE流式输出，元数据过滤不暴露推理过程。Nginx负载均衡按QPS自动伸缩（2-4实例），健康检查自动恢复。Opentelemetry手写埋点，Trace导出Tempo。Prometheus指标+Grafana可视化面板。Loki日志聚合，trace_id精准检索，问题2分钟定位。QQ邮箱告警通知。",
    },
    {
        "heading": "安全体系",
    ...
    {
        "heading": "性能压测数据",
        "content": "日常运行800并发近零错误，平均响应时间1.1秒。极限压测1600并发，可用性99.28%。三级语义缓存把命中查询的成本降至纯调用的约六成。数据库连接池配置：最小20连接，最大50连接每个实例。",
    },
    {
        "heading": "部署架构",
        "content": "Docker Compose编排10+个服务：Gateway多实例、Nginx负载均衡、PostgreSQL 16、Redis 7、Grafana、Prometheus、Tempo、Loki、Alertmanager。Nginx按QPS自动伸缩（2-4实例），健康检查自动剔除故障节点。Cloudflare Tunnel公网暴露，源站零端口暴露。可直接作为K8s部署基础单元。",
    },
    {
        "heading": "技术栈清单",
        "content": "AI与后端：Python 3.11、FastAPI、DeepSeek API、Multi-Agent编排、ReAct循环、RAG多路召回（SQL+Embedding）、Cross-Encoder Rerank、语义路由、可插拔架构、冷热分层记忆、RAGAS评测。数据库：PostgreSQL 16（pgvector + pg_trgm）、Redis 7。运维监控：Docker Compose、Nginx、Prometheus、Grafana、Tempo、Loki、OpenTelemetry。安全：JWT、OAuth2、DFA敏感词过滤、阿里云SMS。前端：Vue 3 + TypeScript + Vite + Pinia。",
    },
    {
        "heading": "模拟支付体系",
...
async def main():
    ...
    # 合并 data/project_kb/*.md：一个文件一条知识，首行 # 为标题，其余为正文
    # （踩坑经历等新知识直接丢 md 文件进该目录即可，无需改本脚本）
    kb_dir = None
    _script_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    for cand in (
        os.path.join(_script_root, "data", "project_kb"),
        os.path.join(os.sep, "app", "data", "project_kb"),
        os.path.join("data", "project_kb"),
    ):
    ...
    if kb_dir:
        for fname in sorted(os.listdir(kb_dir)):
            ...
            if not text.startswith("# "):
                print(f"  [跳过] {fname}: 首行需为 '# 标题'")
                continue
            head, _, body = text.partition("\n")
            chunks.append({"heading": head[2:].strip(),
                           "content": body.strip() or head[2:].strip()})
        print(f"已从 {kb_dir} 读入 {len(chunks) - len(KNOWLEDGE_CHUNKS)} 条 md 知识")
