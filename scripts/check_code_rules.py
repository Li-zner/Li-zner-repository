    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            if nlines > MAX_FUNC_LINES:
                # 函数级豁免：def 行带 # noqa: func-length（主人决策的豁免名单，须写明理由）
                if "# noqa: func-length" in lines[node.lineno - 1]:
                    continue
                violations.append(
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ExceptHandler) and node.body:
            ...
            if only_pass and node.lineno:
                # 行级豁免：except 行带 # noqa: silent-except（惯例性吞异常：CancelledError/
                # 平台差异/并发清理竞态等，豁免须写明理由）
                if "# noqa: silent-except" in lines[node.lineno - 1]:
                    continue
