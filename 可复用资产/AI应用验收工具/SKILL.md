...
- 组合设计说明：[[AI应用验收工具/README|本工具 README]]（MCP 是插座，skill 是操作规程）
- 失败处置引用：[[skill/避坑检查清单/SKILL|避坑检查清单]]（环境 / 编码 / 配置漂移逐条对照）
- 逐阶段断言细节：[[AI应用验收工具/详细验收规则|详细验收规则]]
- 图谱：[[图谱/MOC-MCP生态|MOC-MCP 生态]]、[[图谱/MOC-开发流程与质量|MOC-开发流程与质量]]
