...
for row in ws.iter_rows(min_row=2, values_only=True):
    cid = row[id_col]
    if cid in cc_ids:
        cc_rows.append((cid, row[score_col], row[analysis_col], str(row[resp_col])[:200]))

cc_rows.sort(key=lambda x: x[0])

scores = [s for _, s, _, _ in cc_rows]
...
if low_cases:
    ...
    for cid, score, analysis, resp in low_cases:
        print(f"\n== {cid}: {score}/10 ==")
