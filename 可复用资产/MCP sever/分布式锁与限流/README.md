...
- 机制参考实现：[[可复用代码/README|可复用代码]]（分布式锁+lua / 分布式限流器+lua，含内置自检）
- 上线前对照：[[skill/生产机制自查/SKILL|生产机制自查]]（资金与并发 / 服务治理两项）
- 手写训练：[[skill/面试手写八段训练/SKILL|面试手写八段]]（第 1 段限流、第 5 段分布式锁）
- 验收：[[AI应用验收工具/README|AI 应用验收工具·阶段 2]]（锁互斥 / 误释放拒绝 / 超限拒绝）
- 图谱：[[图谱/MOC-生产机制与稳定性|MOC-生产机制与稳定性]]、[[图谱/MOC-MCP生态|MOC-MCP 生态]]
