...
- 踩坑原文：[[harness/03-参考-项目经验/03-踩坑清单|harness 踩坑清单]]（项目经验的原始记录）
- 检索类坑（#27-#32）的修复版实现：[[MCP sever/知识库双通道检索/README|知识库双通道检索]]、[[MCP sever/语义缓存/README|语义缓存]]、[[MCP sever/模型归因纠错映射/README|模型归因纠错映射]]
- MCP 专项（#33-#38）的方法论：[[skill/mcp-server开发五步法/SKILL|mcp-server 开发五步法]]
- 图谱：[[图谱/MOC-开发流程与质量|MOC-开发流程与质量]]、[[图谱/MOC-RAG与知识工程|MOC-RAG 与知识工程]]
