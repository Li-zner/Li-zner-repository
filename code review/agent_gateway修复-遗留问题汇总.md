...
| # | 问题 | 来源 | 处理建议 | 状态 |
|---|---|---|---|---|
| A1 | `chat_stream_v2` 上帝函数 965 行（含 generate 924 行）未拆分 | v2指点 #21 | ✅ 已完成（2026-09-02）：v2.py 1845→136 行薄壳，业务下沉 app/services/ 9 模块；新旧 SSE 事件流 5 场景逐字节对拍一致；v2.py 已移出编码规范豁免名单 | 已修复 |
...
| A32 | `test_sliding_counter.py` 按旧 API 调 `check_qps(_now=...)`，当前 rate_limit 已移除 `_now` 注入参数（第 3 轮熔断改造后未同步更新测试）→ 5 项单测必挂 | 2026-08-18 C7 全量单测发现 | ✅ 已修复（2026-08-31）：选项① 恢复 `_now` 注入——`rate_limit._QPS_COUNTER_LUA` 加 ARGV[3] 绕过分时源（生产传空串仍走 Redis 服务端 TIME，行为不变）、`check_qps` 加 `_now` kwarg、`fake_redis.py` 滑动窗口分支同步新 arg 形状；单测 5/5 → 全量 40/40 绿 | 已修复 |
| A33 | `tests/conftest.py` 未设 `os.environ.setdefault("APP_ENV","test")`，而 `config.py:131` 用 `APP_ENV != "test"` 豁免 ADMIN_PASSWORD fail-loudly → 裸环境任何 app.* import 直接 raise，全部测试文件无法收集（比「个别失败」更严重，挡的是整套测试） | 2026-08-31 本次实测 | ✅ 已修复（2026-08-31）：conftest 补 `APP_ENV=test`；裸环境测试即可收集运行 | 已修复 |
